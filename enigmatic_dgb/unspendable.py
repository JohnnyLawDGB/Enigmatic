...
def decode_address(address: str) -> Tuple[str, str]:
    """Best-effort decoding of an unspendable address.

    Decoding derives the prefix from the address, validates it against the
    configured categories, and removes padding/checksum characters before
    translating the Base58 digits back to the DiMECASH alphabet via
    :data:`b58_dcmap`. The process is best-effort and may fail for addresses not
    generated by :func:`generate_address`.
    """

    if len(address) < 4:
        raise ValueError("Address is too short to decode.")

    prefix = address[:3]
    prefix_bytes = _prefix_to_version(prefix)

    # Validate that the address can be decoded with the derived prefix. We do
    # not need the payload bytes themselves—this round-trip acts as a checksum
    # and shape check.
    base58_decode(address, prefix_bytes)

    body = address[len(prefix) :]
    if len(body) <= 6:
        raise ValueError("Address body is too short to contain a payload.")

    # The trailing six characters correspond to the checksum segment introduced
    # during Base58Check encoding. Strip them before removing padding.
    body_without_checksum = body[:-6]
    unpadded_body = body_without_checksum.rstrip("X").rstrip("z")
    if not unpadded_body:
        raise ValueError("Decoded body is empty after stripping padding.")

    message_chars: List[str] = []
    for character in unpadded_body:
        try:
            mapping_index = b58_digits.index(character)
        except ValueError as exc:
            raise ValueError(f"Invalid Base58 character in body: {character}") from exc
        message_chars.append(b58_dcmap[mapping_index])

    decoded_message = "".join(message_chars)

    # Attempt to return the original input by replaying the generator with the
    # normalized body and its DiMECASH-decoded variant. The first candidate
    # whose re-encoding matches the provided address is treated as the
    # recovered message.
    for candidate in (unpadded_body, decoded_message):
        try:
            if generate_address(prefix, candidate) == address:
                return prefix, candidate
        except ValueError:
            continue

    return prefix, decoded_message
